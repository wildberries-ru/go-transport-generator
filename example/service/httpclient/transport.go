...
import (
	...
	type UploadDocumentTransport interface {
		EncodeRequest(ctx context.Context, r *fasthttp.Request, token *string,name string,extension string,categoryID string,supplierID *int64,contractID *int64,data *multipart.FileHeader) (err error)
	...
	func (t *uploadDocumentTransport) EncodeRequest(ctx context.Context, r *fasthttp.Request, token *string,name string,extension string,categoryID string,supplierID *int64,contractID *int64,data *multipart.FileHeader) (err error) {
		...
		r.Header.Set("Content-Type", "multipart/form-data")
			...
			writer := multipart.NewWriter(body)
				...
				writer.WriteField("categoryID", _categoryID)
			
				
				
					if contractID) != nil {
						
							_contractID := fmt.Sprintf("%d", *contractID)
						
					}
				
				writer.WriteField("contractID", _contractID)
				...
				writer.WriteField("extension", _extension)
				...
				writer.WriteField("name", _name)
			
				
				
					if supplierID) != nil {
						
							_supplierID := fmt.Sprintf("%d", *supplierID)
						
					}
				
				writer.WriteField("supplierID", _supplierID)
	...
	type GetWarehousesTransport interface {
		...
		DecodeResponse(ctx context.Context, r *fasthttp.Response) (pets map[string]v1.Detail,err error)
	}
	...
	// DecodeResponse method for decoding response on server side
	func (t *getWarehousesTransport) DecodeResponse(ctx context.Context, r *fasthttp.Response) (pets map[string]v1.Detail,err error) {
	...
	func (t *getFileTransport) DecodeResponse(ctx context.Context, r *fasthttp.Response) (data []byte,fileName string,err error) {
		if r.StatusCode() != http.StatusOK {
			...
			application/vnd.openxmlformats-officedocument.spreadsheetml.sheet = ptr(r.Header.Peek("Content-Type"))
